...
async function loadRecovery() {
  ...
  loading.hidden = true;
  if (data.paymentLink?.shortUrl) { complete.hidden = false; document.querySelector('#completeTitle').textContent = 'Your secure payment link is ready.'; document.querySelector('#completeMessage').textContent = `Order ${data.orderId} · ${currency.format(data.amount)}`; const link = document.querySelector('#paymentLink'); link.href = data.paymentLink.shortUrl; link.hidden = false; return; }
  if (data.feedbackSubmitted || data.status !== 'awaiting_customer_feedback') { complete.hidden = false; document.querySelector('#completeMessage').textContent = data.publicMessage || 'Your response is recorded. We will handle the next step according to the merchant’s payment policy.'; return; }
